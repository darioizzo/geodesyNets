...
def compute_c_for_model(
    ...
    predicted = ACC_L(target_points, mascon_cube_points, mascon_cube_masses)
...
def validation_mascon(
    ...
    for idx in range((len(target_points) // batch_size) + 1):
        indices = list(
            range(
                idx * batch_size, np.minimum((idx + 1) * batch_size, len(target_points))
            )
        )
        points = target_points[indices]
        labels.append(ACC_L(points, mascon_points, mascon_masses).detach())
        pred.append(ACC_L(points, mascon_cube_points, mascon_cube_masses).detach())
        if progressbar:
    ...
    for loss_fn in loss_fns:
        if loss_fn in [
            ...
            loss_values.append(torch.mean(loss_fn(pred, labels)).cpu().detach().item())
        else:
            loss_values.append(
    ...
    for idx, altitude in enumerate(sampling_altitudes):
        ...
        for batch in range(N // batch_size):
            target_points = target_sampler().detach()
            labels.append(ACC_L(target_points, mascon_points, mascon_masses).detach())
            pred.append(
                ACC_L(target_points, mascon_cube_points, mascon_cube_masses).detach()
            )
