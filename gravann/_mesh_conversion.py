...
def create_mesh_from_cloud(cloud_points,cube_scale=1,subdivisions = 6, stepsize = 0.005,
    ...
    for i in range(N):
        target_direction[i] = target_direction[i] / np.linalg.norm(target_direction[i])
    ...
    while any(point_to_compute) and it < max_iter:
        ...
        if use_top_k:
            cloud_distances = _point_cloud_topk_distance(new_points,cloud_points)
        else:
...
def create_mesh_from_model(model, encoding, cube_scale=1.5, subdivisions = 6, #mesh subdivs
    ...
    while any(point_to_compute) and it < max_iter:
        ...
        #Compute values at new positions
        new_points = torch.tensor(new_points, dtype=torch.float, device=os.environ["TORCH_DEVICE"])
